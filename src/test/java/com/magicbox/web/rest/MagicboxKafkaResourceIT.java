...
import com.magicbox.config.KafkaProperties;
...
class MagicboxKafkaResourceIT {
    ...
    void setup() {
        ...
        MagicboxKafkaResource kafkaResource = new MagicboxKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/magicbox-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/magicbox-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
